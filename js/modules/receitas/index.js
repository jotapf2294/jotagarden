            ...
            <details id="details-form" style="background: white; padding: 15px; border-radius: 12px; box-shadow: var(--shadow); margin-bottom: 20px; border: 1px solid var(--border);">
                ...
                <form id="form-receita" style="display: grid; gap: 12px; margin-top:15px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px;">
                        <input type="text" id="rec-rend-total" placeholder="Rend. Total (Ex: 2kg)">
                        <input type="number" id="rec-rend-porcoes" placeholder="Nº Porções" value="1">
                        <input type="text" id="rec-tempo" placeholder="Tempo Preparo">
                        <input type="text" id="rec-temp-arm" placeholder="Temp. Armazenamento">
                    </div>

                    <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                        <label style="font-size: 0.75rem; font-weight: bold; color: var(--primary);">INGREDIENTES (PESOS E CUSTOS):</label>
                        <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 40px; gap: 8px; margin-top: 10px;">
                            ...
                            <input type="text" id="inp-un" placeholder="Un (kg/g)">
                            ...
                            <button type="button" id="btn-add-ing" class="btn">+</button>
                        </div>
                        <div id="lista-temp-ing" style="margin-top:10px; display:flex; flex-wrap:wrap; gap:5px;"></div>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                        <textarea id="rec-pcc" placeholder="Pontos Críticos de Controlo (PCC)..." style="height: 60px; border-left: 3px solid #f59e0b;"></textarea>
                        <textarea id="rec-preparo" placeholder="Modo de Preparo detalhado..." style="height: 60px;"></textarea>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                        <input type="text" id="rec-embalagem" placeholder="Tipo de Embalagem">
                        <input type="text" id="rec-validade" placeholder="Validade após produção">
                    </div>
                    <input type="text" id="rec-alergenos" placeholder="Alergénios (Reg. 1169/2011)">

                    <div style="display:flex; gap:10px;">
                        <button type="submit" class="btn btn-primary" style="flex:2; padding: 12px;">GUARDAR FICHA TÉCNICA</button>
...
const setupEvents = (receitas, insumos) => {
    ...
    // ... (restante das funções de pesquisa e renderCards mantêm-se iguais ao anterior)
    const filtrar = () => {
        ...
        const filtradas = receitas.filter(r => (r.nome.toLowerCase().includes(termo)) && (cat === "Todos" || r.categoria === cat));
...
// --- FUNÇÃO DE IMPRESSÃO EXATAMENTE IGUAL À QUE ENVIASTE ---
window.visualizarFicha = async (id) => {
    ...
    printArea.innerHTML = `
        <div class="ficha" style="background: white; border: 1px solid #000; font-family: 'Inter', sans-serif;">
            <div style="background: #1e3a8a; color: white; padding: 15px; display: flex; justify-content: space-between;">
                <h1 style="font-size: 18px; margin:0;">FICHA TÉCNICA DE PRODUÇÃO</h1>
                <div style="font-size: 10px; text-align: right;">Doc: FTP-${r.codigo || '000'}<br>Rev: 01 | ${r.dataAtual}</div>
            </div>

            <div style="padding: 20px;">
                <h2 style="font-size: 12px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0; text-transform: uppercase;">Informações Básicas</h2>
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; font-size: 11px; margin-bottom: 15px;">
                ...
                <h2 style="font-size: 12px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0; text-transform: uppercase;">Ingredientes e Custos</h2>
                <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 10px;">
                    <thead>
                        <tr style="background: #f1f5f9;">
                            <th style="border: 1px solid #e2e8f0; padding: 5px;">Ingrediente</th>
                            <th style="border: 1px solid #e2e8f0; padding: 5px;">PB</th>
                            <th style="border: 1px solid #e2e8f0; padding: 5px;">Un</th>
                            <th style="border: 1px solid #e2e8f0; padding: 5px;">FC</th>
                            <th style="border: 1px solid #e2e8f0; padding: 5px;">PL Usado</th>
                            <th style="border: 1px solid #e2e8f0; padding: 5px;">€ Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${r.ingredientes.map(ing => {
                            ...
                            return `<tr>
                                <td style="border: 1px solid #e2e8f0; padding: 4px;">${ing.nome}</td>
                                <td style="border: 1px solid #e2e8f0; padding: 4px; text-align:center;">${ing.pesoBruto}</td>
                                <td style="border: 1px solid #e2e8f0; padding: 4px; text-align:center;">${ing.un}</td>
                                <td style="border: 1px solid #e2e8f0; padding: 4px; text-align:center;">${ing.fc}</td>
                                <td style="border: 1px solid #e2e8f0; padding: 4px; text-align:center;">${ing.pesoLiquido.toFixed(3)}</td>
                                <td style="border: 1px solid #e2e8f0; padding: 4px; text-align:right;">${custo.toFixed(2)}€</td>
                            </tr>`;
                        }).join('')}
                    </tbody>
                </table>

                <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 10px; margin-bottom: 15px;">
                ...
                <h2 style="font-size: 12px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0; text-transform: uppercase;">Modo de Preparo e Segurança</h2>
                <div style="background: #fef3c7; border-left: 3px solid #f59e0b; padding: 8px; font-size: 10px; margin-bottom: 10px;">
                    <strong>PCC (Pontos Críticos):</strong> ${r.pcc || 'N/A'}
                </div>
                <div style="font-size: 11px; white-space: pre-wrap; margin-bottom: 15px; border: 1px solid #eee; padding: 10px;">${r.preparo}</div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        <h2 style="font-size: 12px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0;">EMBALAGEM</h2>
                        <div style="font-size: 11px;">Tipo: ${r.embalagem}<br>Validade: ${r.validade}</div>
                    </div>
                    <div>
                        <h2 style="font-size: 12px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0;">ALERGÉNIOS</h2>
                        <div style="font-size: 11px; color: #dc2626;">${r.alergenos}</div>
                    </div>
                </div>
            </div>

            <div style="background: #f1f5f9; padding: 15px; font-size: 9px; display: flex; justify-content: space-between; margin-top: 10px;">
    ...
    display.innerHTML = lista.map(r => `
        <div class="card" style="display:flex; justify-content:space-between; align-items:center; padding:12px;">
            ...
            <div style="display:flex; gap:5px;">
                <button class="btn btn-sm" onclick="window.visualizarFicha('${r.id}')">👁️ Ficha</button>
...
window.atualizarListaTemp = atualizarListaTemp;
...
window.eliminarFicha = async (id) => {
    if (confirm("Apagar ficha técnica?")) { await remove('receitas', id); renderReceitas(); }
};
...
window.editarFicha = async (id) => {
    ...
    document.getElementById('rec-rend-porcoes').value = r.rendPorcoes;
